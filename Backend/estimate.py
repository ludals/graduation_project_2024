...
for company in companies:
    ...
    # 마켓 데이터 로드 및 결합
    market_data = pd.read_csv(market_file, encoding='cp949')  # 인코딩을 cp949로 변경
    market_data['Date'] = pd.to_datetime(market_data['Date'])
    market_data_up_to_0822 = market_data[market_data['Date'] <= '2024-08-22']
    
    # 병합할 때 Date를 기준으로 병합합니다
    data_up_to_0822 = pd.merge(data_up_to_0822, market_data_up_to_0822, on='Date', how='left')
    ...
    data_up_to_0822_scaled = scaler.transform(data_up_to_0822[stock_features + market_features].fillna(0))
    input_tensor = torch.tensor(data_up_to_0822_scaled, dtype=torch.float32)

    # 모델 불러오기
    model = TimeSeriesTransformer(input_dim=29, d_model=64, nhead=4, num_layers=4, sequence_length=input_tensor.shape[0], dropout=0.2)
