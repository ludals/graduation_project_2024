...
while True:
    try:
        ...
        df = kiwoom.block_request("opt10081",  # 일봉 데이터 요청
                                  종목코드=code,
                                  기준일자="20230801",  # 기준일자 설정 (필요에 따라 조정)
                                  수정주가구분="1",  # 수정주가 반영
                                  output="주식일봉차트조회",
                                  next=next_request)
...
filtered_df['DateTime'] = pd.to_datetime(filtered_df['DateTime'], format='%Y%m%d')

# 엑셀 파일로 저장
filtered_df.to_csv("005930_daily_data.csv", index=False)
